 ...
 *     @type int $exclude_post_id ID текущей записи.
 * }
 */
$args = wp_parse_args(
    $args ?? [],
...
$excludePostId = max(0, (int) $args['exclude_post_id']);
$popularPosts = PostTypeContent::getPopularPosts(
    $contentType,
    4,
    $excludePostId > 0 ? [$excludePostId] : []
);
...
$sidebarPrefix = $contentType === 'articles' ? 'articles' : 'news';
?>
<aside class="news-single__sidebar banks-list-layout__sidebar" aria-label="<?php esc_attr_e('Дополнительная колонка', 'theme'); ?>">
  <div class="banks-list-layout__sidebar-inner">
    <?php if ($popularPosts !== []) : ?>
      <section class="news-single__popular" aria-labelledby="<?php echo esc_attr($sidebarPrefix); ?>-single-popular-heading">
        <h2 id="<?php echo esc_attr($sidebarPrefix); ?>-single-popular-heading" class="news-single__popular-title">
          <?php echo esc_html($config['popular_title']); ?>
        </h2>
        <div class="news-single__popular-list">
          <?php foreach ($popularPosts as $popularPost) : ?>
            <?php Template::part('template-parts/components/popular-news-item', null, ['post' => $popularPost]); ?>
          <?php endforeach; ?>
        </div>
      </section>
    <?php endif; ?>

    <section class="banks-list-layout__sidebar-card banks-list-layout__sidebar-card--converter" aria-labelledby="<?php echo esc_attr($sidebarPrefix); ?>-single-sidebar-converter-heading">
      <header class="banks-list-layout__sidebar-head banks-list-layout__sidebar-head--converter">
        ...
        <div class="banks-list-layout__sidebar-head-text">
          <h2 id="<?php echo esc_attr($sidebarPrefix); ?>-single-sidebar-converter-heading" class="banks-list-layout__sidebar-title">
      ...
      <div class="banks-list-layout__sidebar-converter" data-sidebar-converter data-rates="<?php echo esc_attr(wp_json_encode($sidebarCbrRates)); ?>">
        ...
        Template::part('template-parts/components/currency-amount-group', null, [
            'input_name' => $sidebarPrefix . '-sidebar-amount-from',
            'options' => $sidebarConverterFromOptions,
            'selected_index' => $sidebarSelectedIndex,
            'select_id' => $sidebarPrefix . '-single-sidebar-currency-from',
        ]);
        ?>
        ...
        <?php
        Template::part('template-parts/components/currency-amount-group', null, [
            'input_name' => $sidebarPrefix . '-sidebar-amount-to',
            'input_readonly' => true,
            'options' => $sidebarConverterToOptions,
            'selected_index' => 0,
            'select_id' => $sidebarPrefix . '-single-sidebar-currency-to',
        ]);
        ?>
      </div>
    </section>

    <section class="banks-list-layout__sidebar-card banks-list-layout__sidebar-card--cities" aria-labelledby="<?php echo esc_attr($sidebarPrefix); ?>-single-sidebar-cities-heading">
      <header class="banks-list-layout__sidebar-head">
        ...
        <h2 id="<?php echo esc_attr($sidebarPrefix); ?>-single-sidebar-cities-heading" class="banks-list-layout__sidebar-title">
