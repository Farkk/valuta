...
use Theme\Helpers\News;

/**
 * @var array<string, mixed> $args {
 *     @type WP_Post|null $post Пост новости.
 * }
 */
$args = wp_parse_args(
    $args ?? [],
...
$post = $args['post'] instanceof WP_Post ? $args['post'] : null;

if ($post === null) {
    return;
}

$card = News::getNewsCardData($post);
?>
<article class="popular-news-item">
  <h3 class="popular-news-item__title">
    <a class="popular-news-item__link" href="<?php echo esc_url($card['url']); ?>">
      <?php echo esc_html($card['title']); ?>
    </a>
  </h3>
  <div class="popular-news-item__meta">
    <?php if ($card['date'] !== '') : ?>
      <?php if ($card['date_iso'] !== '') : ?>
        <time class="popular-news-item__date" datetime="<?php echo esc_attr($card['date_iso']); ?>">
          <?php echo esc_html($card['date']); ?>
        </time>
      <?php else : ?>
        <span class="popular-news-item__date"><?php echo esc_html($card['date']); ?></span>
      <?php endif; ?>
    <?php endif; ?>
    <span class="popular-news-item__views" aria-label="<?php echo esc_attr(sprintf(__('Просмотров: %d', 'theme'), $card['views'])); ?>">
      <span class="popular-news-item__views-icon" aria-hidden="true">
      ...
      <span class="popular-news-item__views-count"><?php echo esc_html((string) $card['views']); ?></span>
    </span>
  </div>
</article>
